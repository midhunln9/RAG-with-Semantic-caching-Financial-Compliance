...
class PineconeRepository(VectorDBProtocol):
    ...
    async def upsert_chunks(self, chunks: List[Document]):
        ...
        if not self.client.has_index(self.pinecone_config.index_name):
            ...
            logger.success(f"Index '{self.pinecone_config.index_name}' created (dimension={dimension})")
        else:
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            metadata = [
                {"source": chunk.metadata["source"], "page": chunk.metadata["page"], "text": chunk.page_content}
                for chunk in chunk_batch
            ]

            dict_vector_chunks = [
                {"id": vec_id, "values": dense_embedding, "sparse_values": sparse_embedding, "metadata": metadata_chunk}
                for vec_id, dense_embedding, sparse_embedding, metadata_chunk
                in zip(ids, dense_embeddings, sparse_embeddings, metadata)
            ]
